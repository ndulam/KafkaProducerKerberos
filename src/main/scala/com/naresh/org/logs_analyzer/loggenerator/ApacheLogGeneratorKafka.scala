package com.naresh.org.logs_analyzer.loggenerator

import java.io.{PrintWriter, DataOutputStream, FileOutputStream}
import java.net.ServerSocket
import java.util.{Calendar, Date}

import com.naresh.org.utils.KafkaHandler
 ...
 * @author ${user.name}
 */
object ApacheLogGeneratorKafka {
  ...
  def main(args : Array[String]) {
    ...
    val n_records_per_day = args(0).toInt
    val kafkaBrokerList = args(1)
    val kafkaTopicName = args(2)
    var kafkaHandler: KafkaHandler = null

    kafkaHandler = new KafkaHandler(kafkaBrokerList, kafkaTopicName)
    val logDataFeedActor = new ApacheLogGeneratorKafka
    logDataFeedActor.initialize
    logDataFeedActor.LogGenerator(d, n_records_per_day.toInt, kafkaHandler)
  }

}


class ApacheLogGeneratorKafka {

  val str: String = "Hello, How are you"
  private val ipA_by_ctry = Array.ofDim[Int](ApacheLogGeneratorKafka.MAX_IPS_PER_COUNTRY,ApacheLogGeneratorKafka.N_COUNTRIES)
  private val ipB_by_ctry = Array.ofDim[Int](ApacheLogGeneratorKafka.MAX_IPS_PER_COUNTRY,ApacheLogGeneratorKafka.N_COUNTRIES)
  private val tot_ips_by_ctry: Array[Int] = new Array[Int](ApacheLogGeneratorKafka.N_COUNTRIES)
  private val cum_hourly_weight_by_ctry = Array.ofDim[Int](24,ApacheLogGeneratorKafka.N_COUNTRIES)
  ...
  private val requests: Array[String] = new Array[String](ApacheLogGeneratorKafka.MAX_FILE_LINES)
  private val referrers: Array[String] = new Array[String](ApacheLogGeneratorKafka.MAX_FILE_LINES)
  private val user_agents: Array[String] = new Array[String](ApacheLogGeneratorKafka.MAX_FILE_LINES)
  ...
  def initialize: Unit = {
    ...
    for (i <- 0 to ApacheLogGeneratorKafka.N_COUNTRIES - 1) {
      tot_ips_by_ctry(i) = 0
      ctry_ind.put(ctry_abbr(i), String.valueOf(i))
    }

    for (i <- 0 to ApacheLogGeneratorKafka.MAX_IPS_PER_COUNTRY - 1) {
      for (j <- 0 to ApacheLogGeneratorKafka.N_COUNTRIES - 1) {
        ipA_by_ctry(i)(j) = 0
        ipB_by_ctry(i)(j) = 0
      }
    }
    ...
    for (line <- Source.fromInputStream(getClass.getResourceAsStream("/all_classbs.txt")).getLines()) {
      ...
      if (fields.length == 2) {
        ...
        if (i_ctry_s != null) {
          ...
          if (tot_ips_by_ctry(i_ctry) < ApacheLogGeneratorKafka.MAX_IPS_PER_COUNTRY) {
  ...
  def weight: Unit= {
    ...
    var hourly_weight_by_ctry = Array.ofDim[Int](24,ApacheLogGeneratorKafka.N_COUNTRIES)


    for( hour <- 0 to 23){
      for(ctry <- 0 to ApacheLogGeneratorKafka.N_COUNTRIES - 1){
  ...
  def LogGenerator(d: Date, n_records_per_day: Int, kafkaHandler: KafkaHandler): Unit = {
    ...
    var month_abbr: Array[String] = Array[String]("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")
         ...
         while(true) {
            ...
            clicks_left = 1 + rand.nextInt(ApacheLogGeneratorKafka.MAX_CLICKS_PER_USER)
          ...
          //  println(" publishing "+output)
            kafkaHandler.publish(output)
